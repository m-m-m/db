...
public abstract class AbstractDbDialect<SELF extends AbstractDbDialect<SELF>> implements DbDialect {
  ...
  /**
   * @param properties the {@link Map} with the configuration parameters for the {@link DbSource}.
   * @param source the {@link DbSource}.
   */
  public void autoConfigure(Properties properties, DbSource source) {

    if (DbSource.KEY_URL.get(properties) == null) {
      ...
      String url = autoConfigureUrl(properties, source, kind);
      if (url == null) {
        throw new ObjectNotFoundException("Property", source.getPropertyKey(DbSource.KEY_URL.getName()));
      }
      properties.put(DbSource.KEY_URL.getName(), url);
    }
  }
