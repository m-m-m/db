...
class AlterTableTest extends DbStatementTest {
  ...
  void testAddColumnCustomConstraint() {
    ...
    Song s = Song.of();
    Person p = Person.of();
    DbColumnReference column = DbColumnReference.of(s.Composer());
    DbColumnReference referenceColumn = DbColumnReference.of(p.Id());
    ...
    AlterTableStatement<Song> alterTableStatement = new AlterTableClause<>(s).addColumn(s.Composer())
        .addConstraint(DbForeignKeyConstraint.of("MY_FK_CONSTRAINT", column, referenceColumn, state)).get();
    // assert
    check(alterTableStatement,
        """
            ALTER TABLE Song
